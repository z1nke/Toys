...
void runServer()
{
    ...
    while (true)
    {
        ...
        ssize_t nr = ::recvfrom(sockfd, &msg, sizeof(msg), 0, reinterpret_cast<sockaddr*>(&cliaddr), &addrlen);
        if (nr == sizeof(msg))
        {
            msg.response = now();
            ssize_t nw = ::sendto(sockfd, &msg, sizeof(msg), 0, reinterpret_cast<const sockaddr*>(&cliaddr), addrlen);
            if (nw < 0)
            {
...
{
    const char* helpInfo = "Usage ./roundtrip [option]:\n-s\tserver\nhost\tclient";
    if (argc != 2)
    {
